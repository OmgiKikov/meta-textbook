...
try:
    from graph_structures import Node, Edge, Metadata, NodeType, RelationType, LevelType
except ImportError:
...
### 1. Determine Overall Context:
Identify the *single main RP Topic* from the list above that is most relevant to the content of this specific textbook passage.

---

### 2. Identify and NORMALIZE Entities:

a. Read the passage and extract all phrases matching the **Definition of an Entity** (see below).  
b. Apply the **Normalization Requirement** strictly to merge synonyms and generalize variants into a single canonical entity.

---

### 3. Assign Attributes for Each Entity:

For each *normalized* entity, assign:

- `id`: Unique ID (e.g., "node_" + lowercase normalized name)  
- `name`: Single normalized term  
- `type`: Choose one of the following types:  
      - `concept`: abstract or key concept (e.g., energy, democracy)  
      - `process`: actions, cycles, processes (e.g., фотосинтез, митоз)  
      - `object`: concrete structures, elements, objects (e.g., митохондрия, ядро)  
      - `person`: person (e.g., Дарвин, Менделеев)  
      - `event`: event (e.g., Октябрьская революция, битва)  
      - `place`: geographical location (e.g., Париж, Сибирь)  
      - `work`: creative work (e.g., Война и мир, Евгений Онегин)  
      - `theory`: theory or law (e.g., теория относительности, закон Архимеда)  


- `level`: Level of concept mastery:   
      - `basic`: "C-grade" level — foundational concepts without which the topic cannot be understood. If a student doesn't understand such a term, they should fail the topic! Example in biology: "Белки", "Клетка"
      - `intermediate`: "B-grade" level — important clarifications, examples, distinctions. Example: "Ферменты" 
      - `advanced`: "A-grade" level — everything an excellent student should know: explanatory constructs, connections between topics.
      - `olympiad`: olympiad level — rare, deep, non-obvious concepts. Example: "Белки теплового шока"
  
- `context`: Sentence that fully describes the passage providing context of a `name` in detail and clearly.
- `topic`: The *main RP Topic title* you identified in Step 1. 
- `subtopic`: The *most relevant subtopic* listed under the chosen main topic (from Step 1) that this entity relates to in the passage. Use null if no specific subtopic fits well.

---

### 4. Identify Relationships Between Entities:

a. Find connections between normalized entities, explicitly or implicitly expressed in the text.  
b. Use **only the following relationship types** and understand them as:

- `is_a`: "is a type/subtype of" — classification (митоз is_a процесс)  
- `part_of`: "is a part of" — structural inclusion (ядро part_of клетка)  
- `explains`: "explains/reveals" — explanatory logic (митоз explains рост)  
- `example_of`: "example of something" — specific case (фотосинтез example_of метаболизм)  
- `contrasts_with`: "contrasts with" — comparison or distinction (мейоз contrasts_with митоз)  
- `leads_to`: "leads to" — cause-effect relationship (мутация leads_to болезнь)  
- `analogous_to`: "analogous to" — metaphor or meaning transfer (мембрана analogous_to кожа)

c. Determine the complexity level of each relationship:
  - `basic`: "C-grade" level — fundamental relationships without which the topic cannot be understood. If a student doesn't understand this connection, they should fail the topic! Example: "Белки" `is_a` "органические вещества"
  - `intermediate`: "B-grade" level — important connections, clarifications, examples, distinctions. Example: "Фермент" `part_of` "метаболизм"  
  - `advanced`: "A-grade" level — explanatory constructs, complex connections between concepts. Example: "Белки" `explains` "каталитическая активность"  
  - `olympiad`: olympiad level — rare, deep, non-obvious connections. Example: "Мутация в белке кристаллина" `leads_to` "катаракта"
  

---

### 5. Assign Attributes for Each Relationship:

- `source`, `target`: Use `id` of normalized entities  
- `relation_type`: One of the 7 types above  
- `level`: `basic`, `intermediate`, `advanced`, or `olympiad`  
- `description`: Quote or paraphrased description of this relationship  
- `topic`: Main RP topic (same as in entities)  
- `subtopic`: Most relevant subtopic from RP (or null)

---

### 6. Format Output:

Return a single JSON object with two lists:
- `"entities"` — list of all normalized entities  
- `"relationships"` — list of all meaningful relationships between them

IMPORTANT: Ensure your JSON is properly formed with correct commas, brackets, and quote marks. Double-check that all required fields are included for each entity and relationship.

---

### Definition of an Entity (Node):

An entity MUST represent a meaningful educational term, process, person, object, place, work, or theory.  
**Avoid** generic section headers or vague descriptions like "role", "characteristic", "importance".

---

### Normalization Requirement (STRICT):

- Merge synonyms (e.g. "белки теплового шока", "шапероны") into one canonical term  
- Generalize specific variants (e.g. "Гемоглобин А" → "Гемоглобин") unless the variant is central to the topic  
- Use the normalized form as both `name` and in `id` (e.g. `node_гемоглобин`)

---

### Handling Edge Cases:

- If the passage is extremely short or lacks clear educational concepts, focus on the few high-quality entities that can be extracted
- If content doesn't clearly map to any RP topic/subtopic, use the most closely related one
- Prioritize quality over quantity - it's better to have fewer well-defined entities than many poor ones

---

### Important Notes:
- The examples provided are in Russian but follow the same principles for any language
- Do not create relationships that aren't supported in the text
- Every entity should be connected to at least one other entity unless it's truly isolated in the text
...
{{
  "entities": [
    {{
      ...
      "type": "process",
      "level": "basic",
      "context": "Митоз — это способ деления клетки, при котором образуются две идентичные клетки.",
      "topic": "Клетка и её компоненты",
      "subtopic": "Деление клетки"
    }}
  ],
  "relationships": [
    {{
      ...
      "level": "advanced",
      "description": "Митоз обеспечивает рост организма.",
      "topic": "Клетка и её компоненты",
      "subtopic": "Функции деления"
    }}
  ]
}}
...
—-----------
Textbook Passage: {chapter_text} 

—--------------
Return ONLY the single valid JSON object adhering to ALL instructions above, with properly normalized entities and typed relationships. Do not include any explanations or text outside the JSON object.
"""
    ...
    try:
        ...
        try:
            ...
            level_type_map = {lt.value: lt for lt in LevelType} if isinstance(LevelType, type(Enum)) else {}
            ...
            DEFAULT_IMPORTANCE = 2 # Default importance score
            ...
            if 'entities' in final_data_dict and isinstance(final_data_dict['entities'], list):
                 ...
                 for entity_dict in final_data_dict['entities']:
                    try:
                        ...
                        level = None
                        level_str = entity_dict.get('level')
                        if level_str and level_type_map:
                            level = level_type_map.get(level_str)
                            if not level:
                                logger.warning(f"Unknown level '{level_str}' received from LLM for entity '{entity_dict.get('id')}'.")
                        ...
                        # --- Importance Mapping from Level ---
                        importance = DEFAULT_IMPORTANCE
                        # Используем level для установки importance, если level есть
                        if level_str:
                            if level_str == 'basic':
                                importance = 5
                                ...
                                importance = 3
                            elif level_str == 'olympiad':
                                importance = 2
                        # Иначе пробуем использовать прямое указание importance
                        else:
                        ...
                        node_data = {
                            ...
                            'type': node_type,
                            'importance': importance, # Use validated or mapped importance
                            'level': level, # Add level to node data
            ...
            if 'relationships' in final_data_dict and isinstance(final_data_dict['relationships'], list):
                 ...
                 for i, rel_dict in enumerate(final_data_dict['relationships']):
                    try:
                        ...
                        # Обработка уровня (level)
                        level = None
                        level_str = rel_dict.get('level')
                        if level_str and level_type_map:
                            level = level_type_map.get(level_str)
                            if not level:
                                logger.warning(f"Unknown level '{level_str}' received from LLM for relationship from '{rel_dict.get('source')}' to '{rel_dict.get('target')}'.")

                        # --- Strength Mapping from Level ---
                        strength = DEFAULT_STRENGTH
                        # Используем level для установки strength, если level есть
                        if level_str:
                            if level_str == 'basic':
                                strength = 5
                            elif level_str == 'intermediate':
                                ...
                                strength = 2
                        # Иначе пробуем использовать прямое указание strength
                        else:
                        ...
                        edge_data = {
                            ...
                            'strength': strength, # Use mapped or validated strength
                            'level': level, # Add level to edge data
