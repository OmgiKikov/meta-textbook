...
def chat_with_gigachat(giga, system_prompt, user_message):
    ...
    res = giga.chat.invoke(messages, temperature=0)
    return res.content
...
def build_graph_from_files(
    ...
    if llm_rp_result_data is None:
        ...
        try:
            ...
            subject = "История России" # temp_subject # Assign to main variables
            grade = "11 класс"# temp_grade
    ...
    # предсохранение
    with open(f'{GRAPH_DIR}/chunk_dict.json', 'w', encoding='utf-8') as f:
    ...
    final_nodes_new, final_edges_new = [], []
    for node in final_nodes:
        final_nodes_new.append(node_to_dict(node))
    for edge in final_edges:
        final_edges_new.append(edge_to_dict(edge))
    
    # Сохранение final_nodes и final_edges в JSON файлы
    nodes_file_path = "nodes.json"
    edges_file_path = "edges.json"

    try:
        with open(nodes_file_path, 'w', encoding='utf-8') as nodes_file:
            json.dump(final_nodes_new, nodes_file, ensure_ascii=False, indent=4)
        print(f"Final nodes successfully saved to {nodes_file_path}")
    except Exception as e:
        print(f"Error saving final nodes to {nodes_file_path}: {e}")
    try:
        with open(edges_file_path, 'w', encoding='utf-8') as edges_file:
            json.dump(final_edges_new, edges_file, ensure_ascii=False, indent=4)
        print(f"Final edges successfully saved to {edges_file_path}")
    except Exception as e:
        print(f"Error saving final edges to {edges_file_path}: {e}")
            
    # --- Step 5.2: Сreate a node description
    giga = GigaChatAPIWrapper()
    for k, node in all_nodes_dict.items():
        node_summary = node.summary
        node_title = node.title
        try:
            if node_summary != "Candidate node from RP.":
                prompt = f"Name: {node_title}\nВспомогательные фрагменты:\n{node_summary}"
                # completion = openrouter_client.chat.completions.create(
                #     model="anthropic/claude-3.7-sonnet",
                #     messages=[
                #         {
                #             "role": "system",
                #             "content": "Твоя задача на руссом написать опредедление термина, для школьного учeника: понятно и профессионально. Тебе так же будут даны фрагменты, где этот термин употребляется. Выведи только определение термина, которые написано в 'Name'.",
                #         },
                ...
                system_prompt = "Твоя задача на руссом написать опредедление термина, для школьного учeника: понятно и профессионально. Тебе так же будут даны фрагменты, где этот термин употребляется. Выведи только определение термина, которые написано в 'Name'. Пиши простым текстом, без *."
                response_content = chat_with_gigachat(giga, system_prompt, prompt)
            else:
                response_content = "Candidate node from RP."
        except:
            response_content = "Candidate node from RP."
        node.description = response_content
        all_nodes_dict[k] = node
